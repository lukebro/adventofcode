...
const getLineEquation = (line: Line): Function => {
    ...
    return (x: number, y: number) => {
        ...
        const straight = line.x1 === line.x2 || line.y1 === line.y2;

        return straight && onLine && minMax;
    };
};
...
export default (file: string) => {
    ...
    const equations = file
        .split('\n')
        .map((line): Line => {
            const matches = findall(/(\d+),(\d+) -> (\d+),(\d+)/g, line).map(
                (n) => parseInt(n, 10),
            );
...
function debug(graph: number[], width) {
    for (let i = 0; i < graph.length; i++) {
        if (i % width === 0 && i !== 0) {
            process.stdout.write('\n');
        }

        process.stdout.write(graph[i] === 0 ? '. ' : graph[i] + ' ');
    }
    console.log();
}
